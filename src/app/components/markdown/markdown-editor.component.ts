...
export class MarkdownEditorComponent implements ControlValueAccessor, Validator {
  ...
  private FILE_MANAGER_ID_MARKDOWN = 'MARKDOWN';
  ...
  ngOnInit() {
    ...
    this.fileService.getFileSelected$().subscribe((data: {id: string, file: BlogFile}) => {
      ...
      if (data.id === this.FILE_MANAGER_ID_MARKDOWN) {
        this.insertContent('NBImage', data.file.id);  
      }
  ...
  insertContent(type: string, imageId?: number) {
    ...
    switch (type) {
      ...
      case 'Link':
        ...
        break;
      case 'Image':
        //this.fileService.showFileManager(this.FILE_MANAGER_ID_MARKDOWN);
        selectedText = `![](http://)`;
        break;
      case 'Ul':
        selectedText = `- ${selectedText || initText}`
      ...
      case 'NBImage':
        //this.fileService.showFileManager(this.FILE_MANAGER_ID_MARKDOWN);
        selectedText = `![](/api/v1/files/${imageId})`;
        this.fileService.hideFileManager(this.FILE_MANAGER_ID_MARKDOWN);
        break;
    }
